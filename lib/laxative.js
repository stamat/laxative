...
// Dev watch: rebuild when the markup source changes. ponytail: full rebuild on
// change (poops is fast); no browser live-reload yet — refresh to see changes.
export function watchAndRebuild(config) {
  const inRel = config.raw.markup?.in
  if (!inRel) return
  const inDir = path.resolve(config.root, inRel)
  let timer
  fsWatch(inDir, { recursive: true }, () => {
    clearTimeout(timer)
    timer = setTimeout(() => buildOnce(config).catch((e) => console.error(e.message)), 300)
  })
...
export function init(dir = process.cwd()) {
  ...
  console.log('   next: npm i septic && npm i -D poops laxative, then `laxative dev`')
}
