...
export function proxyTo(port) {
  return (req, res) => {
    ...
    upstream.on('error', () => {
      res.statusCode = 502
      res.end('poops dev server is not answering (yet) — it builds before it serves')
    })
    req.pipe(upstream)
  }
}
...
export async function conductDev(config) {
  ...
  if (!cli || !config.raw.serve) {
    if (cli && !config.raw.serve) console.log('💊 no "serve" block for poops — falling back to markup watch, no livereload')
    return null
  }
  if (config.septic) {
    const { db } = prepareDb(config.septic)
  ...
  const child = spawn(process.execPath, [cli, '-p', String(port)], { cwd: config.root, stdio: 'inherit' })
