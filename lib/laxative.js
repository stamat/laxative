...
export function init(dir = process.cwd()) {
  ...
  console.log('💊 scaffolded poops.json + src/markup/{index,thanks}.html')
  console.log('   next: npm i -D poops laxative, then `laxative dev`')
}

const STARTER = {
  // includePaths is what lets the page say {% include "messages-form.html" %}:
  // poops skips `_`-prefixed directories when rendering pages, but does not
  // search them for includes unless they are on the engine's include paths.
