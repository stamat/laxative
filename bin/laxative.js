...
// Everything that can fail here — a missing config, a septic or poops build —
// already carries a message written for a person. A stack trace on top of one
// buries it, so the CLI prints the message and exits 1.
try {
  if (cmd === 'init') {
    init(process.cwd())
  } else if (cmd === 'build') {
